...
更新时间：2026-05-23 PST
状态：`external_blocked` / `wave29_checked`。本目录已迁入 `ARCHIVE_EXTERNAL_BLOCKED`；仓内 node manifest/runtime replay 已覆盖 `keyword` / `vector` / `hybrid` provider manifest consumption。早期 `partial` 文案仅作历史分期记录，不再是当前目录主状态。

防误读：当前 canonical decision 以本 `INDEX.md` 与 `08_wave29-oss-node-vector-manifest-replay-2026-05-23.md` 为准。重新进入 `CURRENT_DEV` 前，必须先补齐 live embedding provider verification、local open-search quality、semantic relevance 与 live scheduler/tenant DB/UI SLA。
...
| Live provider / scheduler / tenant DB / UI SLA | external blocker | requires live runtime evidence |
