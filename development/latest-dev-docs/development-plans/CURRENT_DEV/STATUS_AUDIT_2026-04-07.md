...
- `external_blocked` [2026-03-02-ingest-platformization-assessment](../ARCHIVE_EXTERNAL_BLOCKED/2026-03-02-ingest-platformization-assessment/09_wave29-ingest-platformization-repo-local-closure-2026-05-23.md) - fetch-router decomposition、GateService/rule-source、default propagation、replay/SLO 与 frontend/ops entry repo-local gates 已闭；configured-service canary、production 24h readback 与 ops approval 未闭环
- `external_blocked` [2026-03-02-meaningful-ingest-guardrails-plan](../ARCHIVE_EXTERNAL_BLOCKED/2026-03-02-meaningful-ingest-guardrails-plan/10_wave29-source-policy-tuning-attachment-decision-2026-05-23.md) - source-policy tuning attachment 已归入 crawler policy matrix；live canary feedback、production 24h guardrail metrics 与 operations strict-gate promotion decision 未闭环
...
- `external_blocked` [2026-03-05-oss-node-platform-io-plan](../ARCHIVE_EXTERNAL_BLOCKED/2026-03-05-oss-node-platform-io-plan/08_wave29-oss-node-vector-manifest-replay-2026-05-23.md) - OSS-node vector manifest runtime replay repo-local gate 已闭；live embedding provider、local open-search quality、semantic relevance 与 live scheduler/tenant DB/UI SLA 未闭环
- `external_blocked` [2026-05-14-global-vectorization-general-foundation](../ARCHIVE_EXTERNAL_BLOCKED/2026-05-14-global-vectorization-general-foundation/11_wave30-vector-closure-external-blocked-decision-2026-05-23.md) - retrieval run persistence/readback、qdrant/pgvector payload provenance 与 Agent matrix/main-search schema join repo-local blockers 已闭；live embedding provider、semantic quality 与 production vector quality 未闭环
- `external_blocked` [2026-03-01-open-source-platform-integration](../ARCHIVE_EXTERNAL_BLOCKED/2026-03-01-open-source-platform-integration/09_wave30-open-source-external-blocked-decision-2026-05-23.md) - OSS-node slice 与 global-vector repo-local blocker 均已迁出/清零；live provider、local open-search quality、semantic relevance 与外部 runtime/SLA evidence 未闭环

## 结果矩阵

| 目录 | 状态 | 时效标签 | 说明 |
|---|---|---|---|
| `2026-03-01-open-source-platform-integration` | `external_blocked` | `doc_aligned / wave8_checked / wave10_checked / wave12_checked / wave14_checked / wave18_checked / wave19_checked / wave27_checked / wave30_checked` | Wave29 将 OSS-node IO 迁出为外部阻塞；Wave30 清零 global-vector repo-local blocker，本目录不再有独立仓内 blocker；目录迁入 `ARCHIVE_EXTERNAL_BLOCKED`，剩余为 live provider、local open-search quality、semantic relevance 与外部 runtime/SLA evidence |
| `2026-05-14-global-vectorization-general-foundation` | `external_blocked` | `doc_aligned / wave8_checked / wave10_checked / wave12_checked / wave14_checked / wave18_checked / wave19_checked / wave27_checked / wave29_verified / wave30_checked` | Wave29 冻结主搜索 `evidence_hits` 与 `global_vector_object` response-level schema；Wave30 关闭 retrieval runs/branches/hits persistence、stored payload provenance 与 Agent matrix/main-search schema joining repo-local blockers；目录迁入 `ARCHIVE_EXTERNAL_BLOCKED`，剩余为 live embedding provider、semantic quality 与 production vector quality |
| `2026-03-02-ingest-platformization-assessment` | `external_blocked` | `doc_aligned / wave8_verified / wave12_verified / wave14_checked / wave17_verified / wave19_verified / wave27_checked / wave29_checked` | Wave29 repo-local closure checker 证明 fetch-router decomposition、GateService/rule-source、default propagation、replay/SLO 与 frontend/ops entry blockers 清零；目录迁入 `ARCHIVE_EXTERNAL_BLOCKED`，剩余为 live canary、production 24h readback 与 ops approval |
| `2026-03-02-meaningful-ingest-guardrails-plan` | `external_blocked` | `doc_aligned / wave9_verified / wave11_verified / wave12_verified / wave14_checked / wave17_verified / wave19_verified / wave27_checked / wave29_checked` | Wave29 将 source-policy tuning attachment 重新归入 crawler source-policy matrix，仓内 blocker 清零；目录迁入 `ARCHIVE_EXTERNAL_BLOCKED`，剩余为 live canary feedback、production 24h guardrail metrics 与 operations promotion |
...
| `2026-03-05-oss-node-platform-io-plan` | `external_blocked` | `doc_aligned / wave8_checked / wave10_checked / wave12_checked / wave14_checked / wave18_checked / wave19_checked / wave27_checked / wave29_checked` | Wave29 node manifest/runtime replay gate 覆盖 `keyword` / `vector` / `hybrid` provider manifest consumption；目录迁入 `ARCHIVE_EXTERNAL_BLOCKED`，剩余为 live embedding provider verification、local open-search quality、semantic relevance 与 live scheduler/tenant DB/UI SLA |
| `2026-03-07-docs-root-restructuring` | `partial` | `doc_aligned / wave9_checked / wave10_checked / wave11_checked / wave12_checked / wave16_verified / wave17_verified / wave18_verified / wave19_verified / wave20_verified / wave25_verified / wave29_checked / wave30_verified / retained_partial` | `docs/development` 与 `docs/architecture` target roots 已准备，Wave25 已迁移 development-plans main；Wave29 将最后 unsafe broad archive move 拆成 per-file batch；Wave30 target-root navigation drift 已降为 `missing_refs=0`，但 shared navigation / `MERGED_OVERVIEW` drift 仍有 `shared_missing_refs=14` 且 decomposed archive queue 未执行，因此继续留在 `CURRENT_DEV` |
...
| `2026-03-15-frontend-three-layer-rewrite` | `partial` | `doc_aligned / wave8_verified / wave11_verified / wave12_checked / wave14_checked / wave16_verified / wave17_verified / wave18_verified / wave19_verified / wave20_verified / wave30_verified / retained_partial` | Wave8 已补 topology/i18n/theme contract checker，Wave11 已补 layer-shell coverage gate，Wave12 已补 business-string audit，Wave14 已补 static migration boundary，Wave16 已迁移 Agent Chat i18n slice，Wave17 已迁移 Projects page business strings，Wave18 已迁移 Catalog page business strings，Wave19 已迁移 Dashboard page i18n slice，Wave20 已迁移 ProcessPage i18n slice，Wave30 已迁移 CrawlerManagePage i18n slice；全局 business-string audit 仍有 1724 gaps，因此继续留在 `CURRENT_DEV` |
