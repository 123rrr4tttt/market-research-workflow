...
- B phase: persisted projection store for nodes and edges, with direct B-read path.
...
3. Runtime behavior:
- graph APIs read from B as primary path.
...
Graph node standardization A+B is completed for current backend scope, with B as primary read path and persisted edges enabled.
