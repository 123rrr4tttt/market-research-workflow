...
- Runtime evidence: canonical Wave2 A LanceDB smoke passes for `keyword`, `vector`, and `hybrid` in the optional dependency environment; long-run benchmark quality, embedding semantics, and full evidence-contract alignment remain open.
- Regression gate:
