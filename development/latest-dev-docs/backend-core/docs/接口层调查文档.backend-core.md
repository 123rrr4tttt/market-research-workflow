...
> 最后更新：2026-05-22 | 文档索引：`docs/README.md` | 当前路由快照：[`API_ROUTE_INVENTORY_2026-05-22.backend-core.md`](API_ROUTE_INVENTORY_2026-05-22.backend-core.md)
...
- 后端 HTTP 接口总量：当前 runtime APIRoute 快照包含 `253` 个 `/api/v1` 路由，来自 `32` 个 endpoint 模块；其中 `main/backend/app/api/__init__.py` 聚合注册 `30` 个业务 router 模块，`main.py` / `web_ui_routes.py` 仍提供健康检查与地图兼容 API。
...
- project-key 解析结果通过 `X-Project-Key-Resolved`、`X-Project-Key-Source`、`X-Project-Key-Enforcement-Mode` 等响应头暴露，便于排查前端 active project 与后端读取 project 不一致的问题。
...
- 当前 `/api/v1` APIRoute 总量：`253`
- 当前 `/api/v1` endpoint 模块数：`32`
- 当前 `main/backend/app/api/__init__.py` 聚合注册子路由模块数：`30`
- 当前主路由快照：[`API_ROUTE_INVENTORY_2026-05-22.backend-core.md`](API_ROUTE_INVENTORY_2026-05-22.backend-core.md)
- 聚合入口：`main/backend/app/api/__init__.py`，再由 `main/backend/app/main.py` 以 `/api/v1` 前缀挂载。

### 4.1 模块清单（职责与规模）

| 模块 | 前缀 | 路由数 | 核心职责 |
| --- | --- | ---: | --- |
| `admin.py` | `/admin` | 17 | 管理台查询、文档管理、图谱导出/查询、检索历史 |
| `agent_batch.py` | `/agent-batch` | 16 | 批量任务、审批、调度与状态 |
| `agent_chat.py` | `/agent-chat` | 4 | Agent Chat 会话入口与能力查询 |
| `agent_sessions.py` | `/agent-sessions`、`/agent-approvals` | 16 | Agent session 与审批状态 |
| `codex_auth.py` | `/codex-auth` | 5 | Codex OAuth / auth 状态 |
| `config.py` | `/config` | 4 | 运行配置读取/更新/重载 |
| `crawler.py` | `/crawler` | 8 | Crawler 管理与运行态查询 |
...
| `ingest.py` | `/ingest` | 19 | 数据采集入口（政策/市场/新闻/报告/商品/数据 API/来源库/结构化图搜索） |
| `keywords.py` | `/keywords` | 5 | 关键词与统计辅助接口 |
| `llm_config.py` | `/llm-config` | 14 | LLM 服务配置 CRUD（含项目级配置） |
| `llm_report.py` | `/llm-report` | 1 | LLM 报告生成入口 |
| `market.py` | `/market` | 2 | 市场统计与游戏列表查询 |
| `policies.py` | `/policies` | 4 | 政策列表/统计/州级详情/单条详情 |
| `process.py` | `/process` | 7 | 任务队列/任务历史/取消/日志查询 |
| `products.py` | `/products` | 4 | 商品管理 CRUD |
| `project_customization.py` | `/project-customization` | 8 | 项目定制（菜单/工作流/LLM 映射/图谱配置） |
| `projects.py` | `/projects` | 9 | 项目（租户）管理、自动创建、激活/归档/删除 |
| `reports.py` | `/reports` | 1 | 报表导出（HTML/CSV） |
| `resource_pool.py` | `/resource_pool` | 19 | 资源池、站点条目、统一搜索与提取 |
| `search.py` | `/search` | 2 | 搜索查询与索引初始化 |
| `skills.py` | `/skills` | 2 | Skill runtime 能力入口 |
| `source_library.py` | `/source_library` | 11 | 信息源库频道/条目/外部项目/刷新/同步管理；运行主入口已迁至 `/ingest/source-library/run` |
| `stats.py` | `/stats` | 4 | 时间/密度统计接口 |
| `topics.py` | `/topics` | 4 | 主题管理 CRUD |
| `workflow_graph.py` | `/workflow-graph` | 28 | 工作流图模板、版本、编译与运行 |
| `writing.py` | `/writing` | 18 | Writing Workbench 文档、模板、引用、关键词卡与 LLM actions |
| `main.py` | `/health` | 2 | API 健康检查 |
| `web_ui_routes.py` | `/maps` | 1 | 地图资源兼容接口 |

### 4.2 各模块路由清单（按文件）

> 下方细分清单保留 2026-02 调研脉络，行号和部分历史路径可能已漂移；当前路由面以 `API_ROUTE_INVENTORY_2026-05-22.backend-core.md` 与 `main/backend/tests/contract/test_api_route_drift_contract_unittest.py` 为准。
...
| POST | `/api/v1/ingest/news/resource/{resource_id}` | `ingest_news_resource` | `main/backend/app/api/ingest.py` |
| POST | `/api/v1/ingest/subprojects/{subproject_key}/news/{resource_id}` | `ingest_subproject_news_resource` | `main/backend/app/api/ingest.py` |
...
| POST | `/api/v1/ingest/data-api` | `ingest_social_sentiment` | `main/backend/app/api/ingest.py:296` |
...
| POST | `/api/v1/source_library/items/{item_key}/run` | historical `run_item` | 当前 APIRoute 表已移除；执行入口为 `/api/v1/ingest/source-library/run` |
