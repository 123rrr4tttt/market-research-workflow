"""Deterministically generate the normalized P3 C6 evidence fragment.

Root schema: ``mrw.functorial_successor.p3_fragment.v1``.  The generator binds
the P1 C6.1-C6.3 cell digests, exact Program/Plan/operation/owner identities,
implementation/test hashes, redaction/provider/loop observations and the
family-local PostgreSQL worker requirement without running any live provider,
credential, network or canonical write.  Run from ``main/backend``:

    python3.11 scripts/generate_successor_p3_c6_fragment.py

The generator self-tests determinism (two identical builds) and the
``content_digest`` over the canonical fragment body, then verifies the
persisted bytes re-read to the same canonical JSON.
"""
...
FRAGMENT_PATH = EVIDENCE_ROOT / "p3-fragments/C6.json"
...
def _bindings() -> tuple[
    ...
    source_paths = [
        ...
        (EVIDENCE_ROOT / "p1-fragments/C6.json", "p1_fragment"),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/core.py",
            "legacy_donor_c6_1",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/contracts.py",
            "legacy_donor_c6_1_c6_2",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/fake_provider.py",
            "legacy_donor_c6_2",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/native_provider.py",
            "legacy_donor_c6_2",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/json_provider.py",
            "legacy_donor_c6_2",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/services/agent_core/provider_trace.py",
            "legacy_donor_c6_3",
        ),
        (
    ...
    capability_root = (
        REPOSITORY_ROOT / "main/backend/app/successor_runtime/capabilities"
    )
    implementation_paths = [
        (capability_root / "agent_core_c6_common.py", "family_shared_contracts"),
        (capability_root / "agent_core_c6_1.py", "c6_1_contracts"),
        (capability_root / "agent_core_c6_1_program.py", "c6_1_program"),
        (capability_root / "agent_core_c6_1_interpreters.py", "c6_1_interpreters"),
        (capability_root / "agent_core_c6_2.py", "c6_2_contracts"),
        (capability_root / "agent_core_c6_2_program.py", "c6_2_program"),
        (capability_root / "agent_core_c6_2_interpreters.py", "c6_2_interpreters"),
        (capability_root / "agent_core_c6_3.py", "c6_3_contracts"),
        (capability_root / "agent_core_c6_3_program.py", "c6_3_program"),
        (capability_root / "agent_core_c6_3_interpreters.py", "c6_3_interpreters"),
        (
            REPOSITORY_ROOT
        ...
        (
            ...
            "agent_core_c6_worker.py",
            "postgres_worker_store",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/successor_runtime/substrate/postgres/"
            "agent_core_c6_handler.py",
            "store_rehydrated_handler",
        ),
        (
            REPOSITORY_ROOT / "main/backend/app/successor_runtime/substrate/postgres/"
            "agent_core_c6_canary.py",
            "runtime_canary_service",
        ),
        (Path(__file__).resolve(), "evidence_generator"),
    ]
    test_root = REPOSITORY_ROOT / "main/backend/tests/successor_runtime"
    test_paths = [
        (test_root / "test_p3_c6_1_episode.py", "c6_1_episode"),
        (test_root / "test_p3_c6_2_provider.py", "c6_2_provider"),
        (test_root / "test_p3_c6_3_redaction.py", "c6_3_redaction"),
        (test_root / "test_p3_c6_legacy_shadow.py", "c6_legacy_shadow"),
        (test_root / "test_p3_c6_evidence.py", "c6_evidence"),
        (test_root / "test_p3_c6_worker_postgres.py", "c6_postgres_worker"),
        (
            test_root / "test_p3_c6_runtime_canary_postgres.py",
            "c6_runtime_canary",
        ),
    ]
    return (
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Deterministically generate or read-only check the normalized "
            "P3 C6 evidence fragment."
        )
    )
    parser.add_argument(
        "--check",
        action="store_true",
        help=(
            "read-only canonical comparison with the persisted fragment; "
            "never writes or touches file mtime"
        ),
    )
    args = parser.parse_args(argv)

    fragment = build_digested_fragment()
    canonical_text = _canonical_json(fragment) + "\n"
    expected_bytes = canonical_text.encode("utf-8")
    if args.check:
        current_bytes = FRAGMENT_PATH.read_bytes() if FRAGMENT_PATH.exists() else b""
        if current_bytes != expected_bytes:
            print(
                f"C6 fragment drift detected at {FRAGMENT_PATH}",
                file=sys.stderr,
            )
            return 1
        print(f"OK C6 fragment matches {FRAGMENT_PATH}")
        return 0

    FRAGMENT_PATH.parent.mkdir(parents=True, exist_ok=True)
    FRAGMENT_PATH.write_text(canonical_text)
    persisted_bytes = FRAGMENT_PATH.read_bytes()
