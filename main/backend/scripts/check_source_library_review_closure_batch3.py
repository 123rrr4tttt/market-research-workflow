#!/usr/bin/env python3
"""Wave19 deterministic review-batch 3 gate for source-library review closure."""
...
CONTRACT_VERSION = "source_library.review_closure_batch3.v1"
RUN_DIR = Path("development/latest-dev-docs/automation-runs/source-library-review-closure-batch3/2026-05-22")
DEFAULT_ARTIFACT_PATH = RUN_DIR / "review_batch3.json"
...
def _batch3_fixture_queue() -> dict[str, Any]:
    return build_relevance_review_queue(
        project_key="demo_proj",
        item_key="handler.cluster.search_template.batch3",
        query_terms=["industrial robotics procurement", "dual-use suppliers"],
        candidates=[
            "https://mount.example/posts/robotics-defense-roundup",
            "https://directory.example/source/robotics-procurement-directory",
            "https://external.example/reports/robotics-supply-chain-note",
        ],
        candidate_refs={
            "https://mount.example/posts/robotics-defense-roundup": {
                "site_entry_url": "https://mount.example/search?q={{q}}",
                "entry_type": "search_template",
                "domain": "mount.example",
                "entry_domain": "mount.example",
            ...
            "https://directory.example/source/robotics-procurement-directory": {
                "site_entry_url": "https://directory.example/search?q={{q}}",
                "entry_type": "search_template",
                "domain": "directory.example",
                "entry_domain": "directory.example",
                "candidate_source": "mounted_search_chain",
                "site_policy": "keep",
                "search_service": "basic",
            ...
            "https://external.example/reports/robotics-supply-chain-note": {
                "site_entry_url": "https://external.example/source?q={{q}}",
                "entry_type": "external_project_url_pool",
                "domain": "external.example",
                "entry_domain": "external.example",
        ...
        runtime_diagnostics=[
            {
                "site_url": "https://mount.example/search?q={{q}}",
                "domain": "mount.example",
                "site_policy": "keep",
                "search_service": "basic",
                "adapter_capability_status": "review",
            ...
            {
                "site_url": "https://directory.example/search?q={{q}}",
                "domain": "directory.example",
                "site_policy": "keep",
                "search_service": "basic",
                "adapter_capability_status": "allow",
            ...
            {
                "site_url": "https://external.example/source?q={{q}}",
                "domain": "external.example",
                "site_policy": "keep",
                "search_service": "url_pool",
                "adapter_capability_status": "review",
                ...
                "parser_profile_resolved": "fallback_anchor_only",
                "relevance_review_required": True,
                "relevance_review_reason": "external_project_sample_requires_review",
            },
        ],
        errors=[
            {
        ...
        source_surface="checker.fixture.batch3",
    )


def _batch3_decision(entry: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "review_scope": "deterministic_fixture_batch3_only",
        "effect": {
...
def _remaining_gaps() -> dict[str, dict[str, str]]:
    return {
        "human_review": {
            ...
            "reason": "Batch3 fixture decisions are generated; live Current Dev human review evidence is absent.",
        },
        "public_replay": {
            "status": "open",
            "completion_claim": "not_claimed",
        ...
        "live_source_collection": {
            "status": "open",
            "completion_claim": "not_claimed",
            "reason": "Batch3 records source-collection review gaps without running live source collection.",
        },
...
def build_expected_artifact(repo_root: Path | str | None = None) -> dict[str, Any]:
    ...
    batch3_queue = _batch3_fixture_queue()
    entries = [entry for entry in batch3_queue.get("entries") or [] if isinstance(entry, dict)]
    decisions = [_batch3_decision(entry) for entry in entries]
    ...
    return {
        ...
        "batch_id": "wave19-source-library-review-closure-batch3-2026-05-22",
        "artifact_kind": "deterministic_fixture_review_batch3",
        "scope": {
            "source": (
                "Wave12 review queue + Wave14 taxonomy readiness + Wave16/Wave18 deterministic batches "
                "+ batch3 mounted-source fixture queue"
            ),
            "fixture_only": True,
        ...
        "input_contracts": {
            ...
            "batch3_fixture_queue": {
                "contract_version": REVIEW_QUEUE_CONTRACT_VERSION,
                "queue_state": batch3_queue.get("queue_state"),
                "queued_count": batch3_queue.get("summary", {}).get("queued_count"),
                "queue_ids": sorted(queue_ids),
            },
        },
        "review_batch": {
            "state": "closed_for_third_deterministic_fixture_batch",
            "deterministic_batch3_closed": True,
        ...
        "topic_coverage": {
            topic: {"wave19_evidence_doc": _resolve_existing_relative(root, paths).as_posix()}
...
def _compare_artifact(expected: dict[str, Any], actual: dict[str, Any], errors: list[str]) -> dict[str, Any]:
    ...
    _require(
        actual_batch.get("deterministic_batch3_closed") is True,
        errors,
        "artifact must close the third deterministic fixture batch",
    )
    _require(
        actual_batch.get("closed_queue_ids") == expected_batch.get("closed_queue_ids"),
        errors,
        "artifact closed queue ids do not match generated batch3 queue",
    )
    _require(
    ...
    _require(
        ...
        errors,
        "artifact decisions do not match generated batch3 fixture decisions",
    )
...
def _build_doc_check(root: Path, errors: list[str]) -> dict[str, Any]:
    ...
    for topic, candidates in sorted(EVIDENCE_DOCS.items()):
        ...
        _require(bool(text), errors, f"missing Wave19 topic evidence doc: {relative.as_posix()}")
        _require(
            not missing_markers,
            errors,
            f"Wave19 topic evidence doc missing markers: {relative.as_posix()}: {missing_markers}",
        )
    return {
        "docs": rows,
...
def build_check(
    ...
    *,
    ...
    _require(
        upstream_checks["batch3_fixture_queue"].get("queued_count") == 3,
        errors,
        "batch3 fixture queue must contain three review candidates",
    )

    actual_artifact = _load_json(artifact)
    _require(bool(actual_artifact), errors, f"missing or invalid review batch3 artifact: {artifact}")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check Wave19 source-library deterministic review closure batch 3.")
