...
def build_expected_artifact(repo_root: Path | str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "topic_coverage": {
            topic: {"wave18_evidence_doc": path}
...
def _build_doc_check(root: Path, errors: list[str]) -> dict[str, Any]:
    ...
    for topic, relative in sorted(EVIDENCE_DOCS.items()):
        ...
        _require(bool(text), errors, f"missing Wave18 topic evidence doc: {relative}")
        _require(not missing_markers, errors, f"Wave18 topic evidence doc missing markers: {relative}: {missing_markers}")
    return {
        "docs": rows,
