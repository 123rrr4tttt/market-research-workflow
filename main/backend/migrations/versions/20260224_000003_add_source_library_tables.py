...
def upgrade() -> None:
    ...
    if not table_exists(conn, "shared_ingest_channels"):
        op.create_table(
    ...
    if not table_exists(conn, "shared_source_library_items"):
        op.create_table(
    ...
    if not table_exists(conn, "ingest_channels"):
        op.create_table(
            "ingest_channels",
            sa.Column("id", sa.BigInteger(), primary_key=True, autoincrement=True),
            sa.Column("channel_key", sa.String(length=128), nullable=False, unique=True),
    ...
    if not table_exists(conn, "source_library_items"):
        op.create_table(
