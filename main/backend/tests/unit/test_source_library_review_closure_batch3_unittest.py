...
from scripts.check_source_library_review_closure_batch3 import CONTRACT_VERSION
from scripts.check_source_library_review_closure_batch3 import build_check
from scripts.check_source_library_review_closure_batch3 import build_expected_artifact


REPO_ROOT = Path(__file__).resolve().parents[4]


class SourceLibraryReviewClosureBatch3UnitTestCase(unittest.TestCase):
    def test_expected_artifact_closes_third_fixture_batch_and_keeps_live_gaps_open(self) -> None:
        ...
        self.assertTrue(artifact["review_batch"]["deterministic_batch3_closed"])
        self.assertEqual(artifact["review_batch"]["decision_count"], 3)
        self.assertEqual(artifact["input_contracts"]["batch3_fixture_queue"]["queued_count"], 3)
    ...
    def test_checker_validates_committed_artifact_topic_docs_and_gap_register(self) -> None:
        ...
        self.assertTrue(result["governance_scope"]["deterministic_batch3_closed"])
        ...
        self.assertEqual(result["artifact_check"]["decision_count"], 3)
        self.assertEqual(
    ...
    def test_checker_rejects_human_public_source_or_ingest_closure_claims(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            path = Path(tmpdir) / "review_batch3.json"
