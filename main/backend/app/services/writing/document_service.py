...
def _title_to_slug(title: str, fallback: str) -> str:
    import re
    slug = re.sub(r"[^\w\u4e00-\u9fff\-]+", "-", (title or "").strip(), flags=re.UNICODE)
...
def export_document_markdown(*, doc_id: int, project_key: str) -> dict[str, Any]:
    ...
    slug = _title_to_slug(document.get("title", ""), fallback=f"writing-document-{doc_id}")
    return {
        "doc_id": int(doc_id),
        "project_key": project_key,
        "filename": f"{slug}.md",
