...
from ..services.agent_core.business_chain_context import (
...
def _list_source_library_items_for_agent(
    ...
    items = list_effective_items(
        scope="effective", project_key=project_key, include_execution_plan=False
    )
    with _source_library_agent_cache_lock:
        _source_library_agent_cache[key] = (now, [dict(item) for item in items])
        if len(_source_library_agent_cache) > 32:
            oldest_key = min(
                _source_library_agent_cache.items(), key=lambda item: item[1][0]
            )[0]
...
def _build_agent_core_provider() -> Any:
    ...
    provider_mode = (
        str(getattr(settings, "agent_core_model_provider", "auto") or "auto")
        .strip()
        .lower()
    )
    if provider_mode == "codex_cli":
        if not codex_cli_llm_available():
            raise RuntimeError(
                "agent_core_model_provider=codex_cli requires the local codex "
                "CLI and a valid Codex auth session"
            )
        return JsonCoreProvider(
            chat_model_factory=lambda: CodexCliChatModel(
                model=getattr(settings, "codex_cli_llm_model", None),
    ...
    # auto: native OpenAI first; JsonCoreProvider fallback keeps the old behavior.
    return NativeToolCallingCoreProvider(fallback_provider=JsonCoreProvider())
            ...
            return [
                CoreModelStep.tools(
                    CoreToolCall(
                        ...
                        call_id="call-e2e-workbench-read",
                ...
                CoreModelStep.tools(
                    CoreToolCall(
                        ...
                        arguments={
                            ...
                            "provenance": {
                                "scenario": "writing_workbench_selection_e2e",
                                "selection_snapshot": selection_snapshot,
                            },
                        },
                    )
                ...
                CoreModelStep.final(
                    "已读取当前写作工作台文档，并用 replace_range 按选区范围写回改写段落。"
                ),
            ]
    if "source_candidate_review JSON" in text:
        review_context = _extract_e2e_source_candidate_review_context(text)
        decision = (
            str(review_context.get("decision") or "approved").strip() or "approved"
        )
        candidate = (
            review_context.get("candidate")
            if isinstance(review_context.get("candidate"), dict)
            else {}
        )
        if not candidate:
            candidate = {
        ...
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        "decision": decision,
                        "candidate": candidate,
                        "preferred_ingest": review_context.get("preferred_ingest")
                        or "url_pool",
                        "reason": review_context.get("reason")
                        or "用户在候选来源卡片中选择采集。",
                        "idempotency_key": review_context.get("idempotency_key")
                        or "e2e-source-candidate-review",
                    },
                )
            ),
            CoreModelStep.tools(
                    ...
                    arguments={
                        "ingest_payload": {
                            ...
                            "metadata": {
                                ...
                                if isinstance(candidate.get("trust"), dict)
                                else {},
                            },
                        },
                        "async_mode": True,
            ...
            CoreModelStep.final(
                "候选来源已记录为已采集意向，并已通过 ingest.url_pool.submit 提交到 URL-pool 采集边界；可继续从 ingest status 或任务结果查看入库进度。"
            ),
        ]
    if (
        "url-pool" in text.lower()
        or "URL-pool" in text
        or "刚才采集" in text
        or "刚才提交" in text
    ) and any(token in text for token in ("状态", "完成", "检查", "进度")):
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="source.history.read",
                    call_id="call-e2e-source-history-status",
                    arguments={"include_recent_sessions": False, "item_limit": 10},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="ingest.url_pool.status",
                    call_id="call-e2e-url-pool-status",
                    arguments={"limit": 10},
                )
            ),
            CoreModelStep.final(
                "已读取刚才 URL-pool 提交的会话历史和任务事件。当前可见 task event 已写回 Agent session；若尚无项目内正式 evidence，则写作仍应标记为待复核来源。"
            ),
        ]
    if ("刚才采集" in text or "刚才提交" in text or "URL-pool" in text) and (
        "写" in text or "工作台" in text or "草稿" in text
    ):
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="agent_artifact.search",
                    call_id="call-e2e-url-pool-artifact-search",
                    arguments={"query": "ingest.url_pool_submissions", "limit": 5},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="agent_artifact.read",
                    call_id="call-e2e-url-pool-artifact-read",
                    arguments={"artifact_ref": "ingest.url_pool_submissions.json"},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        ...
                        "source_refs": [
                            "agent-artifact:ingest.url_pool_submissions.json",
                            "url:https://example.gov/robotics-policy",
                        ],
                        "provenance": {
                            "scenario": "url_pool_candidate_to_writing_e2e",
                            "source_boundary": "ingest.url_pool.submit",
                        },
                    },
                )
            ),
            CoreModelStep.final(
                "已读取 URL-pool 提交 artifact，并把刚才采集的候选来源作为待复核来源追加到写作工作台草稿。"
            ),
        ]
        ...
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    call_id="call-e2e-source-catalog",
                    arguments={"limit": 8},
                )
            ),
            CoreModelStep.final(
                "当前我读取的是来源库/数据源目录，它们是采集入口，不等同于已经入库的项目资料。"
            ),
        ]
    if any(
        token in text
        for token in ("长任务", "长程", "持续", "多轮", "调查", "追查", "线索")
    ):
        return _e2e_long_task_scripted_steps()
    if material_intent.work_context == "writing" and material_intent.category in {
        "external_discovery",
        "external_ingest",
    }:
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    call_id="call-e2e-writing-external-context",
                    arguments={"query": text[:300], "limit": 8},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
            ...
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        ...
                        "query_variants": [
                            "e2e writing external source candidates",
                            "e2e writing external official evidence",
                        ],
                        "provider": "auto",
                        "language": "en",
                        "matrix_mode": True,
            ...
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        "items": ["e2e.writing.external"],
                        "async_mode": True,
                        "override_params": {"max_items": 1},
                    },
                )
            ),
            CoreModelStep.final(
                "已先读取写作语境和内部项目资料，再规划外部资料发现，并通过 source intake 提交 e2e.writing.external。"
            ),
        ]
    if material_intent.work_context == "writing" and asks_material_supplement:
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="project.context.bundle",
                    call_id="call-e2e-writing-context",
                    arguments={"query": text[:300], "limit": 8},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="writing.document.list",
                    call_id="call-e2e-writing-list",
                    arguments={"limit": 8},
                )
            ),
            CoreModelStep.final(
                "写作补资料已优先查看内部项目资料和写作工作台文档；当前没有直接执行外部采集。"
            ),
        ]
    if asks_material_supplement:
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="project.context.bundle",
                    call_id="call-e2e-material-context",
                    arguments={"query": text[:300], "limit": 8},
                )
            ),
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        "topic": "一般资料补充",
                        "query_terms": ["supplemental evidence"],
                        "max_candidates": 4,
                        "matrix_mode": True,
                    },
                )
            ...
            CoreModelStep.tools(
                CoreToolCall(
                    ...
                    arguments={
                        ...
                        "query_variants": [
                            "e2e supplemental material candidates",
                            "e2e supplemental official data",
                        ],
                        "provider": "auto",
                        "language": "en",
                        "matrix_mode": True,
            ...
            CoreModelStep.final(
                "一般补充资料已先检查内部项目上下文，然后给出外部发现计划；没有直接把资料等同为来源库。"
            ),
        ]
    if "已有资料" in text or "项目库" in text or "项目里有什么数据" in text:
        return [
            CoreModelStep.tools(
                CoreToolCall(
                    tool_name="project.context.bundle",
                    call_id="call-e2e-project-context",
                    arguments={"query": text[:300], "limit": 8},
                )
            ),
            CoreModelStep.final(
                "已读取项目上下文：内部已有资料、结构化数据、图谱、写作文档和来源库采集入口会被分开标注。"
            ),
        ]
    return _e2e_long_task_scripted_steps()
...
def _e2e_long_task_scripted_steps() -> list[CoreModelStep]:
    calls = [
        CoreToolCall(
            tool_name="project.context.bundle",
            call_id="call-e2e-context",
            arguments={"query": "机器人商业化 写作 外部资料", "limit": 8},
        ),
        CoreToolCall(
            ...
            arguments={
                ...
                "evidence_refs": [
                    {"id": "internal.context.bundle", "label": "内部项目资料"}
                ],
                "gap_list": [
                    {"id": "gap.official.robotics", "summary": "缺少官方或监管来源"}
                ],
        ...
        CoreToolCall(
            ...
            arguments={
                ...
                "query_variants": [
                    "e2e robotics policy source candidates",
                    "e2e robotics official regulation evidence",
                ],
                "provider": "auto",
                "language": "en",
                "matrix_mode": True,
        ...
        CoreToolCall(
            tool_name="ingest.source_library.run",
            call_id="call-e2e-intake",
            arguments={
                "items": ["e2e.robotics.baseline"],
                "async_mode": True,
                "override_params": {"max_items": 1, "source_mode": "e2e_scripted"},
            },
        ),
        CoreToolCall(
            tool_name="agent_long_task.stage.update",
            ...
            arguments={
                ...
                "source_intake": [
                    {
                        "item_key": "e2e.robotics.baseline",
                        "task_id": "e2e-task-robotics-baseline",
                    }
                ],
        ...
        CoreToolCall(
            ...
            arguments={
                ...
                "clue_nodes": [
                    {
                        "id": "robotics_policy_source",
                        "label": "机器人政策来源",
                        "kind": "source_candidate",
                        "source_intake_task_id": "e2e-task-robotics-baseline",
                    }
                ],
        ...
        CoreToolCall(
            ...
            arguments={
                ...
                "source_refs": [
                    "source-library:e2e.robotics.baseline",
                    "agent-artifact:e2e-robotics.leads.json",
                ],
                "provenance": {
                    "scenario": "agent_core_real_backend_e2e",
                    "material_flow": "internal_first_then_external_intake",
                },
            },
        ),
        CoreToolCall(
    ...
    return [CoreModelStep.tools(call) for call in calls] + [
        CoreModelStep.final(
            "已完成内部优先检索、外部来源发现、source intake、线索保存和写作工作台草稿写入。"
        )
    ]
...
def _agent_runtime_feature_flags() -> dict[str, bool]:
    return {
        "agent_runtime_v2_enabled": bool(
            getattr(settings, "agent_runtime_v2_enabled", True)
        ),
        ...
        "capability_calls": [
            {
                ...
                "summary": f"legacy agent_batch submitted job_id={job_id}"
                if job_id
                else "legacy agent_batch completed without job_id",
                "result": loop_result,
            }
        ],
        "final_answer": (
...
def _run_agent_runtime_v2_turn(
    payload: AgentChatTurnRequest, *, command: str, project_key: str | None
) -> dict[str, Any]:
    ...
    out = runtime.run_turn(
        ...
        turn_decision_planner=GuardedModelTurnDecisionPlanner()
...
def _prepare_agent_core_session(
    ...
    if payload.session_id:
        ...
        source = (
            "e2e-scripted"
            if bool(
                getattr(settings, "agent_core_e2e_scripted_provider_enabled", False)
            )
            else "user"
        )
...
def _build_agent_core_context(
    *,
    ...
    project_key: str | None,
    stream: bool = False,
    business_chain_snapshot: str | None = None,
) -> dict[str, Any]:
    prior_transcript = _compact_prior_session_transcript(
        service=service, session_id=session_id, current_command=command
    )
    ...
    context: dict[str, Any] = {
        ...
        "project_key": project_key,
        "agent_core_model_provider": str(
            getattr(settings, "agent_core_model_provider", "auto") or "auto"
        ),
    ...
    if business_chain_snapshot:
        context["business_chain_snapshot"] = business_chain_snapshot
    return context
...
def _compact_agent_core_session_context_summary(
...
def _compact_prior_session_transcript(
    *, service: Any, session_id: str, current_command: str
) -> list[dict[str, Any]]:
    try:
...
def _run_agent_core_turn(
    ...
    core_context = _build_agent_core_context(
        ...
        command=command,
        project_key=project_key,
        business_chain_snapshot=(
            business_chain_snapshot_json(project_key=project_key)
            if provider_mode == "codex_cli"
            else None
        ),
    )
    tool_window_message = _agent_core_tool_window_message(command, core_context)
    tool_window = select_core_tool_window(
        message=tool_window_message, tool_specs=all_specs
    )
    ...
    core = AgentCore(
        ...
        tool_specs=list(tool_window.specs),
        policy_tool_specs=all_specs,
    )
    ...
    result = core.run(
        AgentCoreRequest(
            ...
            if payload.require_high_risk_approval
            else "frozen",
        )
    )
    root_task_id = str(session.get("root_task_id") or "").strip() or None
    ...
    bundle = service.get_session_bundle(session_id)
    capability_calls = [
        _capability_call_from_core_tool_result(item) for item in result.tool_results
    ]
...
def _compact_agent_core_run_loop(result: Any) -> dict[str, Any]:
    return {
        ...
        "permission_request": result.permission_request.to_dict()
...
def _compact_agent_artifacts_for_chat(
...
def _continue_agent_core_approval(
    ...
    if (
        str(binding_payload.get("contract_version") or "")
        != "agent_core.permission_request.v1"
    ):
    ...
    if str(approval.get("status") or "") == "pending":
        approval = service.resolve_approval(
            str(approval_id), approved_by=payload.approved_by, approved=True
        )
    if str(approval.get("status") or "") != "approved":
        raise ValueError("approval must be approved before continue")

    session_id = str(
        binding_payload.get("session_id") or approval.get("requester_session_id") or ""
    ).strip()
...
def _iter_agent_core_stream(payload: AgentChatTurnRequest):
    ...
    if not command:
        yield _sse(
            ...
            {
                "status": "error",
                "error": error_response(ErrorCode.INVALID_INPUT, "message is required"),
            },
        )
        return
    if _agent_chat_requires_explicit_project_key(payload):
    ...
    thread = threading.Thread(
        target=_target, name=f"agent-core-stream-{session_id}", daemon=True
    )
    thread.start()
    yield _sse(
        "agent_core.stream_started",
        {"runtime_variant": "agent_core_v3", "session_id": session_id},
    )
    ...
    if result is None:
        yield _sse(
            "agent_core.error",
            {
                "status": "error",
                "error": error_response(
                    ErrorCode.UPSTREAM_ERROR, "agent core stream ended without a result"
                ),
            },
        )
        return
...
def list_agent_chat_capabilities(project_key: str | None = None) -> dict[str, Any]:
    ...
    return ok(
        {
            "items": runtime.list_capabilities(),
            "feature_flags": flags,
            "tool_pool": tool_pool,
        }
    )
...
def continue_agent_chat_approval(
    ...
    try:
        try:
            approval = get_agent_session_service().store.get_approval(
