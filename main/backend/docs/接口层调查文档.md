...
| GET | `/policy-graph.html` | `policy_graph_page` | `main/backend/app/web_ui_routes.py` | 兼容入口，重定向到统一图谱页（legacy 已封存） |
| GET | `/social-media-graph.html` | `social_media_graph_page` | `main/backend/app/web_ui_routes.py` | 兼容入口，重定向到统一图谱页（legacy 已封存） |
...
| `ingest.py` | `/ingest` | 13 | 数据采集入口（政策/市场/新闻/数据 API/报告/商品/电商） |
...
| POST | `/api/v1/ingest/data-api` | `ingest_data_api` | `main/backend/app/api/ingest.py` |
...
| `social-media-visualization.html / graph.html?type=social` | 社交可视化页 | `/api/v1/dashboard/sentiment-*, /api/v1/admin/content-graph, /api/v1/admin/documents/{id}` |
