...
前端页面：`frontend/templates/graph.html?type=social`

说明：`social-media-graph.html` 已封存，入口统一重定向到 `graph.html?type=social`。
...
| **共现关系** | 支持关键词共现 | 不支持 |
| **子图提取** | 支持按主题提取 | 不支持 |
| **前端页面** | `graph.html?type=social` | `market-data-visualization.html` |
