...
function extractCatalogKeys(source) {
  const keys = new Set()
  const namespacePattern = /(?:shell|navigation|settings|shared|agentChat|projects|catalogPage|graphPage):\s*\{([\s\S]*?)\n\s*\}/g
...
function isCatalogKey(value) {
  return /^(shell|navigation|settings|shared|agentChat|projects|catalogPage|graphPage)\.[A-Za-z0-9_.-]+$/.test(value)
}
