> Content moved: this file is now authoritative under `docs/development/development-plans/main/MERGED_DEVELOPMENT_PLANS.md`.
> Previous compatibility path: `development/latest-dev-docs/development-plans/main/MERGED_DEVELOPMENT_PLANS.md`.
...
### 1.1 执行规范补充（2026-03-05）

- 串并行分层：
  - `L0` 串行初始化（契约冻结、基线快照、风险识别）
  - `L1` 并行原子执行（无依赖、无文件冲突）
  - `L2` 串行收敛（冲突合并、联调、统一回归）
- 原子任务强制字段：
  - `目标/输入/输出/验收/depends_on/blocks`
- 模块 IO 契约（强制）：
  - `module_input_vars/module_output_vars/io_mapping/io_boundary`
  - 变量前缀约定：`in_*`（输入）、`out_*`（输出）、`tmp_*`（中间）
- 冲突处理：
  - 并行任务命中同一文件时，立即转串行合并队列，由单一 owner 任务输出最终补丁。
